  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        {/* Debug: Manual AI Analysis Trigger */}
        {isParent && family && (
          <div className="mt-6">
            <button 
              onClick={async () => {
                ...
                for (const child of children) {
                  try {
                    ...
                    if (childConversationResponses && childConversationResponses.length > 0) {
                      console.log(`Found ${childConversationResponses.length} child responses for ${child.profiles.full_name}`);
                      
                      // Get parent assessment
                      const assessment = await getAssessment(child.profiles.id);
                      if (assessment && assessment.parent_responses.short.length > 0) {
                        console.log('Found parent responses, triggering AI analysis...');
                        
                        // Prepare child responses
                        const childResponses = {
                          short: childConversationResponses
                            .filter(r => r.question_type === 'short')
                            .sort((a, b) => a.question_id - b.question_id)
                            .map(r => r.response),
                          long: childConversationResponses
                            .filter(r => r.question_type === 'long')
                            .sort((a, b) => a.question_id - b.question_id)
                            .map(r => r.response)
                        };

                        console.log('Child responses prepared:', childResponses);
                        console.log('Parent responses:', assessment.parent_responses);
                        
                        // Call AI analysis directly
                        try {
                          const { data: analysisResult, error: analysisError } = await supabase.functions.invoke(
                            'analyze-parent-child-relationship',
                            { body: { parentResponses: assessment.parent_responses, childResponses } }
                          );
                          ...
                          if (analysisResult && !analysisError) {
                            ...
                            const { error: updateError } = await supabase
                              ...
                              .update({
                                child_responses: childResponses,
                            ...
                            if (updateError) {
                              ...
                              console.log('Successfully updated assessment with AI analysis');
                              toast({
                                title: "Success!",
                                description: `AI analysis generated for ${child.profiles.full_name}`,
                              });
                            }
                          }
                        } catch (aiError) {
                    ...
                    console.error(`Error processing ${child.profiles.full_name}:`, error);
                  }
                }
              }}
